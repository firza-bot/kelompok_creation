...
class IntelligenceSubmission(models.Model):
    ...
    STAGE_CHOICES = [
        ...
        (1, 'Pendefinisian Model Dataset'),
        (2, 'Pemrosesan Data'),
        (3, 'Perencanaan Model & Refining'),
        (4, 'Pelatihan & Testing Model'),
        (5, 'Refining Model'),
        (6, 'Materi Komunikasi Teknikal'),
        (7, 'Materi Komunikasi Manajemen'),
    ]
    ...
    # === Pipeline Data ===
    # Menyimpan data form untuk 8 tahap Grid 2
    pipeline_data = models.JSONField(blank=True, null=True, default=dict)
    ...
    def progress_percentage(self):
        if self.status in ('completed', 'sent'):
            return 100
        return int((self.current_stage / 7) * 100)

# --- INTEGRASI UIUX ---

class UIUXUser(models.Model):
...
class UIUXProject(models.Model):
    id = models.IntegerField(primary_key=True)
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, null=True)
...
class UIUXIssue(models.Model):
    id = models.IntegerField(primary_key=True)
    project = models.ForeignKey(UIUXProject, on_delete=models.DO_NOTHING, db_column='project_id')
    ...
    story_points = models.CharField(max_length=50, blank=True, null=True)
    ...
    creation_status = models.JSONField(blank=True, null=True)
    
    assignee = models.ForeignKey(UIUXUser, on_delete=models.DO_NOTHING, db_column='assignee_id', related_name='assigned_issues', blank=True, null=True)
    reporter = models.ForeignKey(UIUXUser, on_delete=models.DO_NOTHING, db_column='reporter_id', related_name='reported_issues', blank=True, null=True)
    sprint = models.ForeignKey(UIUXSprint, on_delete=models.DO_NOTHING, db_column='sprint_id', blank=True, null=True)
...
class UIUXAttachment(models.Model):
    id = models.IntegerField(primary_key=True)
    issue = models.ForeignKey(UIUXIssue, on_delete=models.DO_NOTHING, db_column='issue_id')
    user = models.ForeignKey(UIUXUser, on_delete=models.DO_NOTHING, db_column='user_id')
...
# --- LOKAL INTELLIGENCE CREATION ---

class IssueCreationStatus(models.Model):
    uiux_issue_id = models.IntegerField(unique=True)
    realization_status = models.IntegerField(default=0)  # 0 to 100
...
class OrchestrationPhase(models.Model):
    CATEGORY_CHOICES = [
        ('Planning', 'Planning'),
